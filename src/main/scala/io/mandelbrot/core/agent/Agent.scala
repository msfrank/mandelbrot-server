 ...
 */
class Agent(val services: ActorRef) extends LoggingFSM[Agent.State,Agent.Data] with Stash with RegistrationOps {
  import Agent._
  ...
  when (AgentRunning) {
    ...
    case Event(op: RetireAgent, state: AgentRunning) =>
      val tombstone = DateTime.now(DateTimeZone.UTC).plus(state.registration.policy.retentionPeriod.toMillis)
