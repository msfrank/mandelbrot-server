 ...
 *
 */
sealed abstract class Tick(timeInMillis: Long, val samplingRate: SamplingRate) extends Ordered[Timestamp] {
  ...
  val interval = samplingRate.millis
...
class MinuteTick(timeInMillis: Long) extends Tick(timeInMillis, PerMinute) {
