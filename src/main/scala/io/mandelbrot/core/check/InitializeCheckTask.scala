...
class InitializeCheckTask(checkRef: CheckRef,
  ...
  // state
  var status: Option[CheckStatus] = None
  val observations = new mutable.HashMap[ProbeId,Vector[ProbeObservation]]
  var inflight = 0

  override def preStart(): Unit = {
    // get the current condition of the check
    services ! GetStatus(checkRef, generation)
    log.debug("initializing check {}", checkRef)
  }

  def receive = {

    case result: GetStatusResult =>
      status = result.status

      // gather the observation history
      ...
      if (inflight == 0) {
        caller ! InitializeCheckTaskComplete(status, Map.empty)
        context.stop(self)
      }
    ...
    case result: GetObservationHistoryResult =>
      ...
      if (!result.page.exhausted) {
        ...
        inflight = inflight - 1
        if (inflight == 0) {
          caller ! InitializeCheckTaskComplete(status, observations.toMap)
          context.stop(self)
        }
      }
...
case class InitializeCheckTaskComplete(status: Option[CheckStatus], observations: Map[ProbeId,Vector[ProbeObservation]])
