...
class CheckSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender with WordSpecLike with ShouldMatchers with BeforeAndAfterAll {
  ...
  "A Check" should {
    ...
    "update behavior" in {
      ...
      val initializeCheckStatus1 = stateService.expectMsgClass(classOf[InitializeCheckStatus])
      val status1 = CheckStatus(DateTime.now(), CheckKnown, None, CheckHealthy, Map.empty, None, None, None, None, false)
      stateService.reply(InitializeCheckStatusResult(initializeCheckStatus1, Some(status1)))
      ...
      val initializeCheckStatus2 = stateService.expectMsgClass(classOf[InitializeCheckStatus])
      val status2 = CheckStatus(DateTime.now(), CheckKnown, None, CheckHealthy, Map.empty, None, None, None, None, false)
      stateService.reply(InitializeCheckStatusResult(initializeCheckStatus2, Some(status2)))
    ...
    "change behaviors" in {
      ...
      val initializeCheckStatus1 = stateService.expectMsgClass(classOf[InitializeCheckStatus])
      val status1 = CheckStatus(DateTime.now(), CheckInitializing, None, CheckUnknown, Map.empty, None, None, None, None, false)
      stateService.reply(InitializeCheckStatusResult(initializeCheckStatus1, Some(status1)))
      ...
      val initializeCheckStatus2 = stateService.expectMsgClass(classOf[InitializeCheckStatus])
      val status2 = CheckStatus(DateTime.now(), CheckInitializing, None, CheckUnknown, Map.empty, None, None, None, None, false)
      stateService.reply(InitializeCheckStatusResult(initializeCheckStatus2, Some(status2)))
